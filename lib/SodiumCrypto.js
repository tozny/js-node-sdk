...
export default class SodiumCrypto extends Crypto {
  ...
  async encryptLargeFile(fileName, ak /* , key */) {
    ...
    // Console.log("header: " + headerUint8)
    ...
    let chunk = 0
    while ((chunk = await reader.readAsync()) !== null) {
      ...
      if (chunk !== null && chunk.length === BLOCK_SIZE) {
        ...
        // Console.log("fileblock: " + encryptedFileBlock)
    ...
    //   // console.log('chunk buffer', chunk instanceof Buffer)

    //   if (chunk !== null && chunk.length === BLOCK_SIZE) {
    //     console.log("PASSES IF")
    //     let encryptedFileBlock = sodium.crypto_secretstream_xchacha20poly1305_push(
    //       state,
    //       /*fileBlockUnint8*/ chunk,
    //       null,
    //       TAG_MESSAGE
    //     )
    //     hash.update(encryptedFileBlock)
    ...
    console.log('PAST READING + WRITING')

    // Const checkSum = hash.digest('').toString('base64')
    // const check = checkSum.toString('utf8')
    // const checkSum = hash.digest('base64')
    const checkSum = hash.base64('')
    console.log('checksum: ' + checkSum)
    // Console.log("file name: " + encryptedFileName)
    // console.log("check sum: " + checkSum)
    // console.log("encrypted length: " + encryptedLength)
