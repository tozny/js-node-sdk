...
export default class Crypto extends CryptoBase {
  ...
  async encryptNote(note, accessKey) {
    await sodium.ready
    ...
    for (let key in note.data) {
      if (note.data.hasOwnProperty(key)) {
        encryptedData[key] = await this.encryptField(note.data[key], accessKey)
      }
    }
  ...
  async decryptNote(encrypted, accessKey) {
    await sodium.ready
    ...
    for (let key in encrypted.data) {
      if (encrypted.data.hasOwnProperty(key)) {
        decryptedData[key] = await this.decryptField(encrypted.data[key], accessKey)
      }
    }
